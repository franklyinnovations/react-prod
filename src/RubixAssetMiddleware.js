	...
	return (req, res, next) => {
		...
		switch (process.env.NODE_ENV) {
			case 'development':
				...
				res.locals.app_stylesheets = `<script src="${static_path}/assets/js/dist/${mainjs}"></script>`;
				res.locals.app_scripts = `<script src="${static_path}/assets/js/dist/plugins.js"></script><script src="${static_path}/assets/js/dist/app.js"></script>`;
				break;
			default:
				res.locals.app_stylesheets = `<link rel="stylesheet" href="/css/${maincss}"/>`;
				res.locals.app_scripts = '<script src="/js/dist/plugins.js"></script><script src="/js/dist/app.js"></script>';
				break;
		}
		next();
	};
