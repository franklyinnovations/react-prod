	...
	switch(action.type) {
		case 'LOAD_MS_CREATOR':
			return {
				...
				results: action.meta.results.map(tag => ({
					value: tag.id,
					label: tag.tagdetails[0].title,
				})),
				remarks: action.meta.remarks.map(tag => ({
					value: tag.id,
					label: tag.tagdetails[0].title,
				})),
			};
...
function marksheet(state = null, action) {
	switch(action.type) {
		...
		case 'UPDATE_MS_REMARK': {
			...
			marksheetrecords[action.studentIndex].remark = action.value;
			return {
				...state,
				marksheetrecords,
			};
		}
		case 'UPDATE_MS_RESULT': {
			let marksheetrecords = [...state.marksheetrecords];
			marksheetrecords[action.studentIndex].result = action.value;
			return {
				...state,
				marksheetrecords,
			};
