...
function viewState(state = null, action) {
	switch(action.type) {
		case 'INIT_MODULE':
		case 'VIEW_ADMIN_CLINIC_JOB_LIST':
			return 'LIST';
		case 'START_ADD_CLINIC_JOB':
		case 'SET_ADMIN_CLINIC_JOB_EDIT_DATA':
			return 'DATA_FORM';
		case 'SET_ADMIN_CLINIC_JOB_VIEW_DATA':
			return 'VIEW_DATA';
		default:
			return state;
	}	
...
function item(state = defaultItem, action) {
	switch(action.type) {
		...
		case 'UPDATE_CLINIC_JOB_DATA_VALUE':
			let newState = {...state};
			if(action.name) {
				newState[action.name] = action.value;
			}
