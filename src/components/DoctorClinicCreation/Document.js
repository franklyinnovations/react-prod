...
export default class Document extends React.Component {
    ...
    render() {
        ...
        if(this.props.state.resetDocumentForm) {
            ...
            <Row key="doctor-document">
                ...
                <Col sm={12}>
                    <Row className='gallery-view'>
                        { this.props.state.hospitalFiles.images.map(this.getDataRow, this) }
                        { this.props.state.hospitalFiles.images.length === 0 && this.getNoDataRow(__, "image") }
                    </Row>
                </Col>
                <Col sm={12}>
                    <h3>{__('Videos')}</h3>
                    ...
                    <Row>
                        <Col sm={12}>
                            <Table bordered responsive>
                                ...
                                <tbody>
                                    { this.props.state.hospitalFiles.videos.map(this.getDataVideo, this) }
                                    { this.props.state.hospitalFiles.videos.length === 0 && this.getNoDataRow(__, "video") }
                                </tbody>
                            </Table>
                        </Col>
                    </Row>
    ...
    async saveImage() {
        ...
        data.append('hospitalId',this.props.state.basicDetails.id);
        this.props.state.dispatch(
            actions.doctor_new_clinic_add.saveImage(this.props.state, data)
        );
    }
    ...
    async saveVideo() {
        ...
        data.append('hospitalId',this.props.state.basicDetails.id);
        this.props.state.dispatch(
            actions.doctor_new_clinic_add.saveImage(this.props.state.data, data)
        );
    }
    ...
    changeDocumentStatus(itemId, status, itemType) {
        this.props.state.dispatch(
            actions.doctor_new_clinic_add.changeDocumentStatus(
                this.props.state,
                itemId,
                status,
                itemType
