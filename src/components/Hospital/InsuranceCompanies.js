...
export default class Membership extends React.Component {
	constructor(props) {
        ...
        this.handleDataUpdate = event => {
            let obj;
            this.props.helperData.insurance_companies_tags.map((item) => {
        ...
        this.addNewTag = value => {
            ...
            if(newTag) this.props.dispatch( actions.hospital_profile.customTagAdd(this.props, newTag, this.tagtypeIds.InsuranceCompaniesTagId) )
        }
    }
        ...
        return (
            <div key={'insurance_companies'}>
                ...
                <FormGroup controlId="insurance_companies">
                    <ControlLabel>{__('Insurance Companies')}</ControlLabel>
                    <Select
                        isCreatable={true}
                        ...
                        isLoading={this.props.newTagCreationStates.insuranceTagLoading}
                        options={this.props.helperData.insurance_companies_tags}/>
                    <HelpBlock>{''}</HelpBlock>
                </FormGroup>
                <Well>
                    <div className="Select--multi">
                        { this.props.additionalInfo.insurance_companies.length === 0 && <p>{__('No record')}</p> }
                        {
                            this.props.additionalInfo.insurance_companies.map(item => 
                                <div key={'spec-tag-label-'+item.value} className="Select-value tags-close " style={{marginLeft: '0px'}}>
                                    {this.props.basicDetails.is_live != 1 && <span className="Select-value-icon" data-itemName={'insurance_companies'} data-d={JSON.stringify(item)} data-itemValue={item.value} onClick={this.handleRemoveTag}>×</span>}
