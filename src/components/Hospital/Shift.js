...
export default class Shift extends React.Component {
    ...
    save(){
        ...
        this.state.days.map(function (value, index){
            ...
            if(_ref.state[`ref_${value.key}`] && (_ref.state[`shift_1_from_${value.key}_val`] < 0 || !_ref.state[`shift_1_to_${value.key}_val`] || !_ref.state[`shift_2_from_${value.key}_val`] || !_ref.state[`shift_2_to_${value.key}_val`] )) {
                isError = true;
                return false;
            }
        ...
        if(!isError) { 
            ...
            if(!_.isEmpty(this.props.hospitalTimings)){
            ...
            if(this.props.basicDetails.is_freeze == 1) {
                this.props.dispatch({
                    type: 'UPDATE_TAB_KEY',
                    key: 'add_doctors'
                });
                ...
                this.props.dispatch(
                    actions.hospital_profile.saveTime(this.props.data, finalArr)
                );
            }
    ...
    managefreeze(name, value) {
        ...
        this.props.dispatch(
            actions.hospital_profile.manage_freeze(
                this.props,
                value
            )
        )
    ...
    saveData(event) {
        ...
        this.props.dispatch(
            actions.hospital_profile.saveShifTiming(this.props.data, finalArr, 'doc-fee-timings-info-form')
        );
    }
