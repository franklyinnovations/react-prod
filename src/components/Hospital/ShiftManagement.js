...
export default class ShiftManagement extends React.Component {
      ...
      this.func_copy_week_days = event => {
        ...
        if(value === true){
          ...
          if(this.state.shift_1_from_mons_val !== undefined && this.state.shift_1_to_mons_val !== undefined 
            ...
            Messenger().post({
              type: 'error',
              message: __("Please enter monday\'s entry first")
            });
            return false;
          }
        } else {
            ...
            <tbody>
              ...
              { _ref.state.days.map(function(value, index){
                  return <tr key={index}>
                            ...
                            <td>
                              <FormGroup validationState={ ( _ref.props.errors[`${value.key}`] !== undefined && _ref.props.errors[`${value.key}`].shift_1_from_time) ? "error" : ""}>
                                <RSelect
                            ...
                            <td>
                              <FormGroup validationState={ ( _ref.props.errors[`${value.key}`] !== undefined && _ref.props.errors[`${value.key}`].shift_1_to_time) ? "error" : ""}>
                                <Select
                                    disabled={!_ref.state[`ref_${value.key}s`]}
                            ...
                            <td>
                              <FormGroup validationState={ ( _ref.props.errors[`${value.key}`] !== undefined && _ref.props.errors[`${value.key}`].shift_2_from_time) ? "error" : ""}>
                                <Select
                                    disabled={!_ref.state[`ref_${value.key}s`]}
                            ...
                            <td>
                              <FormGroup validationState={ ( _ref.props.errors[`${value.key}`] !== undefined && _ref.props.errors[`${value.key}`].shift_2_to_time) ? "error" : ""}>
                                <Select
                                    disabled={!_ref.state[`ref_${value.key}s`]}
  ...
  save(){
    ...
    if(this.props.type === "doctor"){
      this.props.dispatch(
        actions.hospital_profile.addDoctorTime(this.props, finalArr)
      );
    } else if(this.props.type === "hospital"){
      this.props.dispatch(
        actions.hospital_profile.linkClinicTime(this.props, finalArr)
      );
    }
  ...
  changeOpenTime(name, value) {
    ...
    this.props.dispatch(
      actions.hospital_profile.changeOpen24X7(
              this.props,
              value
              )
      )
