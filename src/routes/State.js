...
import reducer from '../redux/reducers/state';
import * as actions from '../redux/actions/state';
addView('state', reducer);
...
export default class State extends React.Component {
	...
	permissions = moduleActions(this.props.session.modules, 'state');
	toggleFilters = () => {
						...
						{this.props.item === null && <Loading/>}
						{
							...
							<React.Fragment>
								...
								<FormGroup
									...
									<ControlLabel><Text>Code</Text></ControlLabel>
									<FormControl
										name='code'
										type='text'
										placeholder={__('Code')}
		...
		return (
			<View.Filters search={this.search} remove={this.toggleFilters}>
				...
				<FormControl
					...
					onChange={this.updateFilter}
					placeholder={__('Short Name')}
					value={filterValue(filters, 'state__alias', '')} />
				<FormControl
					type='text'
				...
				<Select
					title={__('Status')}
					name='state__is_active'
					onChange={this.updateFilter}
					options={getStatusOptions(__)}
					placeholder={__('Select Status')}
					value={filterValue(filters, 'state__is_active', null)}/>
			</View.Filters>
		);
	}
				...
				<DataTable>
					<thead>
						<tr>
							...
							<td className='tw-20'><Text>Code</Text></td>
	...
	renderDataRows(__) {
		...
		return this.props.items.map(item => (
			<tr key={item.id}>
				...
				<td className='tw-20'>{item.statedetails[0].name}</td>
				<td className='tw-20'>{item.alias}</td>
				<td className='tw-20'>{item.code}</td>
