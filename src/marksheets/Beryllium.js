...
export default class Beryllium extends React.Component {
	updateSettings = event => {
		if (event.currentTarget.name === 'midTermExams') {
			let value = getInputValue(event.currentTarget);
			value = value[value.length - 1];
			if (this.props.settings.annualExams.indexOf(value) !== -1) {
				messenger.post({
					type: 'error',
					message: window.__('This is already a annual exam.'),
				});
				return;
			}
		}

		if (event.currentTarget.name === 'annualExams') {
			let value = getInputValue(event.currentTarget);
			value = value[value.length - 1];
			if (this.props.settings.midTermExams.indexOf(value) !== -1) {
				messenger.post({
					type: 'error',
					message: window.__('This is already a mid term exam.'),
				});
				return;
			}
		}
		...
		return (
			<Row>
				<Col xs={12}>
					...
					<FormGroup validationState={errors.midTermExams ? 'error' : null}>
						<ControlLabel><Text>Mid term exams</Text></ControlLabel>
						<Select
							multi
							name='midTermExams'
							onChange={updateSettings}
							value={settings.midTermExams}
							options={this.props.options.examheads}/>
						<HelpBlock>{errors.midTermExams}</HelpBlock>
					</FormGroup>
	...
	static validateSettings(_settings) {
		...
		if (settings.midTermExams.length === 0)
			errors.midTermExams = __('This is a required field.');
